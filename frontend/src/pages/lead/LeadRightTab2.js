...
import LeadRightTab2Action1 from './LeadRightTab2Action1.js';
import LeadRightTab2ContextualAction1 from './LeadRightTab2ContextualAction1.js';
import LeadRightTab2ContextualAction2 from './LeadRightTab2ContextualAction2.js';
...
const LeadRightTab2 = ReactRouterDOM.withRouter(function ({ lead }) {
  ...
  const loadCount = async function () {
    try {
      ...
      setCount(await ApiService.getCountLeadAddresses(args));
    }
    catch (error) {
      setError(error);
      return;
  ...
  const loadRecords = async function () {
    ...
    try {
      ...
      args.limit = pageSize;
      records = await ApiService.getLeadAddresses(args);
    }
    catch (error) {
      setError(error);
      return;
    ...
    setItems(records.map((record, index) => ({
      key: record.leadAddressId,
      data: [
        protect(function ([street, city, state, zipCode, countryName]) { return `${street}, ${city}, ${state} ${zipCode}, ${countryName}` }, [ record.street, record.city, record.state, record.zipCode, record.countryName ]),
      ],
      record: record
    })));
  }
  ...
  const submitAction0 = async function (e) {
    ...
    if (isValid(bodyRefAction0.current)) {
      try {
        await ApiService.postLeadAddress(actionData);
      }
      catch (error) {
        setError(error);
        return;
  ...
  const handleContextualAction0 = async function (e, item) {
    ...
    try {
      leadAddress = await ApiService.getLeadAddress(item.record.leadAddressId);
    }
    catch (error) {
      setError(error);
      return;
    }
    setContextualAction({ index: 0, leadAddress: leadAddress });
    const data = {};
    data.street = leadAddress.street;
    data.city = leadAddress.city;
    data.state = leadAddress.state;
    data.zipCode = leadAddress.zipCode;
    data.countryId = leadAddress.countryId;
  ...
  const submitContextualAction0 = async function (e) {
    ...
    if (isValid(bodyRefContextualAction0.current)) {
      try {
        await ApiService.putLeadAddress(contextualAction.leadAddress.leadAddressId, contextualActionData);
      }
      catch (error) {
        setError(error);
        return;
  ...
  const handleContextualAction1 = async function (e, item) {
    ...
    setContextualAction({ index: 1, leadAddress: leadAddress });
  }
  const submitContextualAction1 = async function (e) {
    ...
    if (isValid(bodyRefContextualAction1.current)) {
      try {
        await ApiService.deleteLeadAddress(contextualAction.leadAddress.leadAddressId);
      }
      catch (error) {
        setError(error);
        return;
    ...
    <div>
      <div>
        <div className="pb-1 d-flex align-items-center border-bottom">
          ...
          <ActionsBar
            actions={[
              { icon: IconAdd, label: words.addAddress, color: 'blue', onClick: handleAction0, hidden: !((SecurityService.hasRole('seller'))) },
            ]}
          />
        </div>
        <Grid
          ...
          fields={[
            { icon: IconLeadAddress, label: words.address, type: 'string' },
          ]}
          items={items}
        />
      ...
      {action && action.index === 0 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconAdd} color="blue" label={words.addAddress} />
          </ReactBootstrap.Modal.Header>
          <ReactBootstrap.Modal.Body ref={bodyRefAction0} className="p-0">
            <div className="p-2">
              {actionData && <LeadRightTab2Action1 lead={lead} data={actionData} updateData={updateActionData} validated={actionValidated} />}
            </div>
      ...
      {contextualAction && contextualAction.index === 0 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconEdit} color="blue" label={words.editAddress} />
          </ReactBootstrap.Modal.Header>
          <ReactBootstrap.Modal.Body ref={bodyRefContextualAction0} className="p-0">
            <div className="p-2">
              {contextualActionData && <LeadRightTab2ContextualAction1 leadAddress={contextualAction ? contextualAction.leadAddress : null} data={contextualActionData} updateData={updateContextualActionData} validated={contextualActionValidated} />}
            </div>
          </ReactBootstrap.Modal.Body>
          <ReactBootstrap.Modal.Footer className="p-2">
            <div>
      ...
      {contextualAction && contextualAction.index === 1 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconDelete} color="red" label={words.deleteAddress} />
          </ReactBootstrap.Modal.Header>
          <ReactBootstrap.Modal.Body ref={bodyRefContextualAction1} className="p-0">
            <div className="p-2">
              <LeadRightTab2ContextualAction2 leadAddress={contextualAction ? contextualAction.leadAddress : null} />
            </div>
          </ReactBootstrap.Modal.Body>
          <ReactBootstrap.Modal.Footer className="p-2">
            <div>
