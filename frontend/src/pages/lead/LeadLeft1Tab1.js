...
import LeadLeft1Tab1Action1 from './LeadLeft1Tab1Action1.js';
...
const LeadLeft1Tab1 = ReactRouterDOM.withRouter(function ({ lead }) {
  ...
  const loadCount = async function () {
    try {
      ...
      setCount(await ApiService.getCountInteractions(args));
    }
    catch (error) {
      setError(error);
      return;
  ...
  const loadRecords = async function () {
    ...
    try {
      ...
      args.limit = pageSize;
      records = await ApiService.getInteractions(args);
    }
    catch (error) {
      setError(error);
      return;
  ...
  const submitAction0 = async function (e) {
    ...
    if (isValid(bodyRefAction0.current)) {
      try {
        await ApiService.postLeadInteraction(lead.leadId, actionData);
      }
      catch (error) {
        setError(error);
        return;
    ...
    <div>
      <div>
        <div className="pb-1 d-flex align-items-center border-bottom">
          ...
          <ActionsBar
            actions={[
              { icon: IconAdd, label: words.addInteraction, color: 'blue', onClick: handleAction0, hidden: !((SecurityService.hasRole('seller'))) },
            ]}
          />
        </div>
        <Grid
          ...
          fields={[
            { icon: IconInteraction, label: words.datetime, type: 'datetime' },
            { type: 'string', translate: true, dockAs: 'secondary' },
            ...
            { label: words.change, type: 'percentage', color: function (value) { return value > 0 ? 'green' : value < 0 ? 'red' : 'black'; }, style: function (value) { return value != 0 ? 'fw-bold' : ''; } },
          ]}
          items={items}
        />
      ...
      {action && action.index === 0 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconAdd} color="blue" label={words.addInteraction} />
          </ReactBootstrap.Modal.Header>
          <ReactBootstrap.Modal.Body ref={bodyRefAction0} className="p-0">
            <div className="p-2">
              {actionData && <LeadLeft1Tab1Action1 lead={lead} data={actionData} updateData={updateActionData} validated={actionValidated} />}
            </div>
