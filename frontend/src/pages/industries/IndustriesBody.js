...
import IndustriesBodyAction1 from './IndustriesBodyAction1.js';
import IndustriesBodyContextualAction1 from './IndustriesBodyContextualAction1.js';
import IndustriesBodyContextualAction2 from './IndustriesBodyContextualAction2.js';
import ApiService from '../../services/ApiService.js';
import { getWords } from '../../utils/get-words.js';
import { isValid } from '../../utils/is-valid.js';

const IndustriesBody = ReactRouterDOM.withRouter(function ({  }) {
  ...
  const loadCount = async function () {
    try {
      setCount(await ApiService.getCountIndustries(null));
    }
    catch (error) {
      setError(error);
      return;
  ...
  const loadRecords = async function () {
    ...
    try {
      ...
      args.limit = pageSize;
      records = await ApiService.getIndustries(args);
    }
    catch (error) {
      setError(error);
      return;
  ...
  const submitAction0 = async function (e) {
    ...
    if (isValid(bodyRefAction0.current)) {
      try {
        await ApiService.postIndustry(actionData);
      }
      catch (error) {
        setError(error);
        return;
  ...
  const handleContextualAction0 = async function (e, item) {
    ...
    let industry = null;
    try {
      industry = await ApiService.getIndustry(item.record.industryId);
    }
    catch (error) {
      setError(error);
      return;
    }
    setContextualAction({ index: 0, industry: industry });
    const data = {};
  ...
  const submitContextualAction0 = async function (e) {
    ...
    if (isValid(bodyRefContextualAction0.current)) {
      try {
        await ApiService.putIndustry(contextualAction.industry.industryId, contextualActionData);
      }
      catch (error) {
        setError(error);
        return;
  ...
  const handleContextualAction1 = async function (e, item) {
    ...
    setContextualAction({ index: 1, industry: industry });
  }
  const submitContextualAction1 = async function (e) {
    ...
    if (isValid(bodyRefContextualAction1.current)) {
      try {
        await ApiService.deleteIndustry(contextualAction.industry.industryId);
      }
      catch (error) {
        setError(error);
        return;
    ...
    <div>
      <div>
        <div className="pb-1 d-flex align-items-center border-bottom">
          <Title level={1} icon={IconIndustry} color="blue" label={words.industries} />
        ...
        <Grid
          ...
          fields={[
            { icon: IconIndustry, label: words.name, type: 'string' },
          ]}
          items={items}
        />
      ...
      {action && action.index === 0 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconAdd} color="blue" label={words.addIndustry} />
          </ReactBootstrap.Modal.Header>
          <ReactBootstrap.Modal.Body ref={bodyRefAction0} className="p-0">
            <div className="p-2">
              {actionData && <IndustriesBodyAction1 data={actionData} updateData={updateActionData} validated={actionValidated} />}
            </div>
      ...
      {contextualAction && contextualAction.index === 0 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconEdit} color="blue" label={words.editIndustry} />
          </ReactBootstrap.Modal.Header>
          <ReactBootstrap.Modal.Body ref={bodyRefContextualAction0} className="p-0">
            <div className="p-2">
              {contextualActionData && <IndustriesBodyContextualAction1 industry={contextualAction ? contextualAction.industry : null} data={contextualActionData} updateData={updateContextualActionData} validated={contextualActionValidated} />}
            </div>
          </ReactBootstrap.Modal.Body>
          <ReactBootstrap.Modal.Footer className="p-2">
            <div>
      ...
      {contextualAction && contextualAction.index === 1 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconDelete} color="red" label={words.deleteIndustry} />
          </ReactBootstrap.Modal.Header>
          <ReactBootstrap.Modal.Body ref={bodyRefContextualAction1} className="p-0">
            <div className="p-2">
              <IndustriesBodyContextualAction2 industry={contextualAction ? contextualAction.industry : null} />
            </div>
          </ReactBootstrap.Modal.Body>
          <ReactBootstrap.Modal.Footer className="p-2">
            <div>
