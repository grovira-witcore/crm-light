...
const UsersBody = ReactRouterDOM.withRouter(function ({  }) {
  ...
  const loadRecords = async function () {
    ...
    try {
      ...
      args.limit = pageSize;
      records = await ApiService.getUsersV2(args);
    }
    catch (error) {
      setError(error);
      return;
  ...
  const submitContextualAction0 = async function (e) {
    ...
    if (isValid(bodyRefContextualAction0.current)) {
      try {
        await ApiService.putUser(contextualAction.user.userId, contextualActionData);
      }
      catch (error) {
        setError(error);
        return;
    ...
    <div>
      <div>
        <div className="pb-1 d-flex align-items-center border-bottom">
          <Title level={1} icon={IconUser} color="blue" label={words.users} />
          <div className="ps-4">
            <FiltersBar
              filters={[
                {
        ...
        <Grid
          ...
          fields={[
            { icon: IconDummy, label: words.username, type: 'string' },
            { type: 'string', dockAs: 'secondary' },
            { type: 'imageUrl', dockAs: 'avatar' },
            { label: words.email, type: 'string' },
            { label: words.team, type: 'string' },
            { label: words.enabled, type: 'boolean' },
          ]}
          onClickItem={(SecurityService.hasRole('administrator') || SecurityService.hasRole('seller')) && (SecurityService.hasRole('administrator')) && (SecurityService.hasRole('administrator')) ? handleClickItem : null}
          items={items}
        />
      ...
      {action && action.index === 0 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconAdd} color="blue" label={words.addUser} />
      ...
      {contextualAction && contextualAction.index === 0 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconEdit} color="blue" label={words.editUser} />
          </ReactBootstrap.Modal.Header>
