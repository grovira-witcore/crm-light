...
import MeetingLeftAction1 from './MeetingLeftAction1.js';
import MeetingLeftContextualAction1 from './MeetingLeftContextualAction1.js';
...
const MeetingLeft = ReactRouterDOM.withRouter(function ({ meeting }) {
  ...
  const loadCount = async function () {
    try {
      ...
      setCount(await ApiService.getCountMeetingLeadContacts(args));
    }
    catch (error) {
      setError(error);
      return;
  ...
  const loadRecords = async function () {
    ...
    try {
      ...
      args.limit = pageSize;
      records = await ApiService.getMeetingLeadContacts(args);
    }
    catch (error) {
      setError(error);
      return;
    ...
    setItems(records.map((record, index) => ({
      key: record.meetingLeadContactId,
      data: [
        protect(function ([firstName, lastName]) { return `${firstName} ${lastName}` }, [ record.leadContactFirstName, record.leadContactLastName ]),
        record.leadContactAvatar,
      ],
      record: record
    })));
  }
  ...
  const submitAction0 = async function (e) {
    ...
    if (isValid(bodyRefAction0.current)) {
      try {
        await ApiService.postMeetingLeadContact(actionData);
      }
      catch (error) {
        setError(error);
        return;
  ...
  const handleContextualAction0 = async function (e, item) {
    ...
    let meetingLeadContact = null;
    try {
      meetingLeadContact = await ApiService.getMeetingLeadContact(item.record.meetingLeadContactId);
    }
    catch (error) {
      setError(error);
      return;
    }
    setContextualAction({ index: 0, meetingLeadContact: meetingLeadContact });
  }
  const submitContextualAction0 = async function (e) {
    ...
    if (isValid(bodyRefContextualAction0.current)) {
      try {
        await ApiService.deleteMeetingLeadContact(contextualAction.meetingLeadContact.meetingLeadContactId);
      }
      catch (error) {
        setError(error);
        return;
    ...
    <div>
      <div>
        <div className="pb-1 d-flex align-items-center border-bottom">
          <Title level={1} icon={IconLeadContact} color="orange" label={words.leadParticipants} />
          <div className="flex-grow-1" />
          <ActionsBar
            actions={[
              { icon: IconAdd, label: words.add, color: 'blue', onClick: handleAction0, hidden: !((SecurityService.hasRole('seller'))) },
            ]}
          />
        </div>
        <Grid
          ...
          fields={[
            { label: words.contact, type: 'string' },
      ...
      {action && action.index === 0 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconAdd} color="blue" label={words.addLeadParticipant} />
          </ReactBootstrap.Modal.Header>
          <ReactBootstrap.Modal.Body ref={bodyRefAction0} className="p-0">
            <div className="p-2">
              {actionData && <MeetingLeftAction1 meeting={meeting} data={actionData} updateData={updateActionData} validated={actionValidated} />}
            </div>
      ...
      {contextualAction && contextualAction.index === 0 ?
        ...
        >
          <ReactBootstrap.Modal.Header className="pt-2 pb-1 ps-2 pe-3" closeButton={true}>
            <Title level={1} icon={IconDelete} color="red" label={words.deleteLeadParticipant} />
          </ReactBootstrap.Modal.Header>
          <ReactBootstrap.Modal.Body ref={bodyRefContextualAction0} className="p-0">
            <div className="p-2">
              <MeetingLeftContextualAction1 meetingLeadContact={contextualAction ? contextualAction.meetingLeadContact : null} />
            </div>
          </ReactBootstrap.Modal.Body>
          <ReactBootstrap.Modal.Footer className="p-2">
            <div>
