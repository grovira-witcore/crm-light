...
const Header = ReactRouterDOM.withRouter(function ({ icon, label, languages, menu }) {
  ...
  return (
    <div>
      {/* Header */}
      <div className="p-2 d-flex align-items-center justify-content-between header text-white">
        <div class="d-flex">
        ...
        <div class="d-flex align-items-center">
          ...
          <ReactBootstrap.Dropdown>
            <ReactBootstrap.Dropdown.Toggle className="d-flex align-items-center border-0 header-action">
              {languages.filter((language) => language.code === i18n.code).map((language) => (
                <div key={language.code} className="px-2 d-flex align-items-center">
                  <div className="image-sm mx-auto bg-gray-lighter">
                  {userData.avatar ?
                    <img src={userData.avatar} alt="Image" className="img-fluid rounded-circle" /> :
                    <div/>
                  }
                  </div>
                  <div className="ps-2 fw-bold">{userData.firstName + ' ' + userData.lastName}</div>
                </div>
              ))}
            </ReactBootstrap.Dropdown.Toggle>
            <ReactBootstrap.Dropdown.Menu>
