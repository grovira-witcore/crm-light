    ...
    if (organizedItems[parentFullKey + '|' + level.key]) {
      for (const item of organizedItems[parentFullKey + '|' + level.key]) {
        ...
        renders.push(
          <tr
            key={fullKey}
            className={level.selectedKey !== item.key ? (current && current.item.levelKey === item.levelKey && current.item.key === item.key && (current.childLevelKey === null || current.childLevelKey === undefined) ? 'bg-current' + (hasChildren || level.onClickItem ? ' cursor-pointer' : '') : '') : 'bg-selected'}
  ...
  const renderItemsTitledLevel = function (organizedItems, levelIndex, indent, parentItem) {
    ...
    renders.push(
      <tr
        ...
        className={(current && current.item.levelKey === parentItem.levelKey && current.item.key === parentItem.key && current.childLevelKey === level.key ? 'bg-current' + (hasChildren ? ' cursor-pointer' : '') : '')}
